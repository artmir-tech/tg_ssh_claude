...
18 сценариев: новая сессия, продолжение, изоляция, 5 параллельных задач и очередь, очередь внутри сессии,
...
панель темы, чистота General, голосовое → расшифровка → Claude (если установлен голос). Используется настоящий Claude (модель haiku — дёшево) и поддельный Telegram.
...
- Видео (кроме кружков) не поддерживаются; файлы и голосовые — до 20 МБ (ограничение Bot API).
