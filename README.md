...
- **Одна тема = одна сессия.** Создали тему и написали задание — появилась новая сессия Claude Code.
  Или напишите прямо в General — бот спросит «✉️ Куда отправить?»: в новую тему или в одну из последних.
...
- **Новая сессия:** создайте тему в группе и напишите задание. Или «➕ Новая» на дашборде. Или просто напишите
  в General (текст, голос, файл) → «✉️ Куда отправить?» → «🆕 Новая тема» или одна из последних тем;
  следующие 5 минут сообщения из General идут туда же без вопроса («✖ Не отправлять в …» — выбирать снова).
...
- **Интерфейс.** Панель темы (`panel_view`) и дашборд (`render_view`) — «мини-приложения в одном сообщении»:
  ...
  обычное сообщение (текст, голос, файл) бот держит в памяти и спрашивает «Куда отправить?» (`route_view`),
  затем пересылает его в выбранную тему так, будто оно написано там (`route_to` → `_topic`),
...
панель темы, чистота General, голосовое → расшифровка → Claude (если установлен голос), файлы в обе стороны, режимы разрешений (Авто — на Sonnet), сообщение из General → тема.
