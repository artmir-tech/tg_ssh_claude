...
Кроме того, есть голосовой ввод.
И планирую добавить работу с файлами в ближайшее время.
...
- **Файлы.** Во время хода бот запоминает пути из инструмента `Write` и после ответа отправляет финальные версии
  ...
  инструмент `send_file` (встроенный MCP-сервер SDK `claude_control`, работает внутри бота); разрешение на него
  решает `can_use_tool`: рабочая папка — сразу, секреты — отказ, прочее — кнопки. Альбомы Telegram приходят
...
19 сценариев: новая сессия, продолжение, изоляция, 5 параллельных задач и очередь, очередь внутри сессии,
...
панель темы, чистота General, голосовое → расшифровка → Claude (если установлен голос), файлы в обе стороны. Используется настоящий Claude (модель haiku — дёшево) и поддельный Telegram.
